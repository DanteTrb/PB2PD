...
# CONFIG
# =========================
st.set_page_config(page_title="PB2PD Screening Tool", page_icon="🚶🏻‍♀️‍➡️", layout="wide")
...
DEFAULT_PI_GUIDE_MD = """
# PI Guide — Prospective Enrollment (Prodromal Burden (>= 3) → PD)

**Purpose.** Support selection of *prodromal burden-positive* subjects (RBD/hyposmia/depression/constipation) for prospective follow-up.

## Inclusion/Exclusion (research)
- Inclusion: Prodromes present; independent ambulation; age ≥ 45 y; informed consent.
...
> Note: probability is *of prodromal burden “parkinson-like”*, not PD diagnosis.
...
def make_pdf(
    ...
    *,
    ...
    c.drawString(left, y, "Prodromal burden “parkinson-like” Risk — MVP")
    y -= 0.7*cm
    ...
    c.drawString(left, y, f"PB+ probability: {proba:.3f}")
...
with st.sidebar:
    ...
    screening_on = st.checkbox("Enable Screening mode (pilot on PB-positive cohort)", value=True)
    ...
    if st.button("PB-positive preset (prevalence≈40%, rule-in 0.60)"):
...
with tab1:
    ...
    with c3:
        ...
        hy = 2  # fixed hidden value for model compatibility
        st.write("")
    ...
    if st.button("Compute risk", type="primary"):
        try:
            ...
            st.markdown(f"### PB+ probability: **{p:.3f}**")
            ...
            band_msg = {
                "Green":  "Low suspicion of PD (in PB-positive cohort).",
            ...
            if HAVE_PDF:
                ...
                buf = make_pdf(
                    ...
                    cohort_note="PB+ cohort — research/triage use",
                    # >>> NEW: extra args
                    model_id="RF-v1.0",
                ...
                if buf:
                    st.download_button("Download PDF", data=buf, file_name="PB_report.pdf", mime="application/pdf")
            else:
...
with tab2:
    ...
    visible_features = [f for f in FEATURES if f != "H-Y"]
    st.code(", ".join(visible_features))
    ...
    with colT:
        if st.button("Download CSV template"):
            ...
            tmpl.loc[0] = [1.80, 65, 1.52, 1.30, 75, 70, 1, 1.10, 5]
    ...
    if upl:
        try:
            ...
            # >>> hidden stage
            if "H-Y" in out.columns:
                out = out.drop(columns=["H-Y"])
            out["proba"] = proba
            out["band"] = bands
            st.dataframe(out.head(50))
            st.download_button("Download results CSV", data=out.to_csv(index=False),
                               file_name="PB_batch_results.csv", mime="text/csv")
        except Exception as e:
